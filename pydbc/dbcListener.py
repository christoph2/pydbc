...
# from pydbc import parser  # Moved to local import to avoid circular dependency or missing module
...
def _get_base_listener():
    try:
        from pydbc.parser import BaseListener

        return BaseListener
    except ImportError:
        # Fallback if parser is not available, e.g. when moving to C++ extensions only
        class BaseListenerFallback:
            def __init__(self, *args, **kwargs):
                pass

        return BaseListenerFallback


class DbcListener(_get_base_listener()):
    """ """
    ...
    def bake_queries(self):
        self.MESSAGE_BY_MESSAGE_ID = self.bakery(
            lambda session: session.query(Message).filter(
                Message.message_id == bindparam("message_id")
            )
        )
        self.ENVVAR_BY_NAME = self.bakery(
            lambda session: session.query(EnvVar).filter(
                EnvVar.name == bindparam("name")
            )
        )
        self.ENVDATA_BY_NAME = self.bakery(
            lambda session: session.query(EnvironmentVariablesData).filter(
                EnvironmentVariablesData.name == bindparam("name")
            )
        )
        self.MESSAGE_SIGNAL_BY_NAME = self.bakery(
            lambda session: session.query(Signal.rid)
            .join(Message_Signal)
            .join(Message)
            .filter(
                Message.message_id == bindparam("messageID"),
                Signal.name == bindparam("signal_name"),
            )
        )
        self.NETWORK_BY_RID = self.bakery(
            lambda session: session.query(Network).filter(
                Network.rid == bindparam("rid")
            )
        )
        self.SIGNAL_BY_RID = self.bakery(
            lambda session: session.query(Signal).filter(Signal.rid == bindparam("rid"))
        )
        self.EXISTS_ATTRIBUTE_DEFINITION = self.bakery(
            lambda session: session.query(literal(True)).filter(
                Attribute_Definition.name == bindparam("name")
            )
        )
        self.EXISTS_ATTRIBUTE_VALUE = self.bakery(
            lambda session: session.query(literal(True)).filter(
                Attribute_Value.object_id == bindparam("rid"),
                Attribute_Value.attribute_definition_id == bindparam("ad"),
            )
        )
        self.EXISTS_NETWORK = self.bakery(
            lambda session: session.query(literal(True)).filter(
                Network.name == bindparam("name")
            )
        )
        self.EXISTS_NODE_RXSIGNAL = self.bakery(
            lambda session: session.query(literal(True)).filter(
        ...
        self.EXISTS_VALUE_DESCRIPTION = self.bakery(
            lambda session: session.query(literal(True)).filter(
                Value_Description.valuetable_id == bindparam("rid"),
                Value_Description.value == bindparam("value"),
            )
        )
        self.MESSAGE_SIGNAL_BY_NAME2 = self.bakery(
            lambda session: session.query(Signal)
            .join(Message_Signal)
            .join(Message)
            .filter(
                Message.message_id == bindparam("messageID"),
                Signal.name == bindparam("signalName"),
            )
        )
    ...
    def get_signal_by_name(self, messageID, signal_name):
        res = (
            self.MESSAGE_SIGNAL_BY_NAME(self.session)
            .params(messageID=messageID, signal_name=signal_name)
            .first()
        )
        if res:
            return res.rid
            ...
            self.logger.error(
                "Message signal '{}'::'{}' does not exist.".format(
                    messageID, signal_name
                )
            )
            return []
    ...
    def insertReceivers(self, messageId, signalId, receiver):
        for rcv in receiver:
            ...
            if res:
                ...
                self.logger.error(
                    "While inserting signal receivers: node '{}' does not exist.".format(
                        rcv
                    )
                )
                continue
            exists = (
                self.EXISTS_NODE_RXSIGNAL(self.session)
                .params(nodeId=nodeId, messageId=messageId, signalId=signalId)
                .first()
            )
            if exists:
                self.logger.error(
                    "While inserting signal receivers: receiver messageId: {} signalId: {} nodeId: {} already exists.".format(
                        messageId, signalId, nodeId
                    )
                )
            else:
                rxs = Node_RxSignal(
                    node_id=nodeId, message_id=messageId, signal_id=signalId
                )
    ...
    def insertValueDescription(self, rid, description):
        ...
        for desc, value in description:
            exists = (
                self.EXISTS_VALUE_DESCRIPTION(self.session)
                .params(rid=rid, value=value)
                .first()
            )
            if exists:
                pass
                ...
                vd = Value_Description(
                    valuetable_id=rid, value=value, value_description=desc
                )
                objs.append(vd)
    ...
    def insertAttributeDefinitions(self, ctx):
        ...
        for attr in ctx.value:
            ...
            exists = (
                self.EXISTS_ATTRIBUTE_DEFINITION(self.session).params(name=name).first()
            )
            if exists:
                self.logger.error(
                    "While inserting attribute definitions: an attribute definition named '{}' already exists.".format(
                        name
                    )
                )
            else:
    ...
    def insertAttributeDefaults(self, ctx):
        ...
        for item in ctx.items:
            ...
            defaults[name] = value
            ad = (
                self.ATTRIBUTE_DEFINITION_BY_NAME(self.session)
                .params(name=name)
                .first()
            )
            if not ad:
                self.logger.error(
                    "While inserting attribute default values: attribute '{}' does not exist.".format(
                        name
                    )
                )
                continue
    ...
    def insertNetwork(self, specific=None):
        ...
        if exists:
            self.logger.error(
                "While inserting network: an network named '{}' already exists.".format(
                    name
                )
            )
            return
    ...
    def exitMessageTransmitters(self, ctx):
        ...
        for transmitter in ctx.value:
            msg = (
                self.MESSAGE_BY_MESSAGE_ID(self.session)
                .params(message_id=transmitter["messageID"])
                .first()
            )
            if not msg:
                self.logger.error(
                    "While inserting message transmitters: message '{}' does not exist.".format(
                        transmitter["messageID"]
                    )
                )
                continue
            for name in transmitter["transmitters"]:
                ...
                if not node:
                    self.logger.error(
                        "While inserting message transmitters: node '{}' does not exist.".format(
                            name
                        )
                    )
                    continue
    ...
    def exitMessageTransmitter(self, ctx):
        ...
        ctx.value = dict(
            messageID=self.getValue(ctx.messageID), transmitters=transmitters
        )
    ...
    def exitSignalExtendedValueTypeList(self, ctx):
        ...
        for item in valueTypes:
            ...
            if not signal:
                self.logger.error(
                    "While inserting signal extended value types: signal '{}' does not exist.".format(
                        srid
                    )
                )
                continue
    ...
    def exitSignalExtendedValueType(self, ctx):
        ...
        if not valType in (0, 1, 2, 3):
            self.logger.error(
                "While parsing signal extended value type: value type must be in range [0..3] - got '{}', using 0.".format(
                    valType
                )
            )
            valType = 0
    ...
    def exitMessages(self, ctx):
        ...
        for msg in ctx.value:
            ...
            if not tx_node:
                self.logger.error(
                    "While inserting messages: node '{}' does not exist.".format(
                        transmitter
                    )
                )
                continue
            ...
            for signal in signals:
                ...
                ms = Message_Signal(
                    offset=startBit,
                    multiplexor_signal=multiplexorSignal,
                    multiplex_dependent=multiplexDependent,
                    multiplexor_value=multiplexorValue,
                    signal=ss,
                    message=mm,
                )
                self.session.add(ms)
    ...
    def exitMessage(self, ctx):
        ctx.value = dict(
            messageID=self.getValue(ctx.messageID),
            name=self.getValue(ctx.messageName),
            dlc=self.getValue(ctx.messageSize),
            transmitter=self.getTerminal(ctx.transmt),
    ...
    def exitSignal(self, ctx):
        ...
        if not byteOrder in (0, 1):
            self.logger.error(
                "While parsing signal: Error while parsing signal '{}': byteorder must be either 0 or 1 -- using 0".format(
                    ctx.signalName.value
                )
            )
            byteOrder = 0
    ...
    def exitBitTiming(self, ctx):
        ctx.value = dict(
            baudrate=self.getValue(ctx.baudrate),
            btr1=self.getValue(ctx.btr1),
            btr2=self.getValue(ctx.btr2),
        )
    ...
    def exitObjectValueTables(self, ctx):
        ...
        for table in ctx.value:
            ...
            if tp == "SG":
                ...
                if not env_var:
                    self.logger.error(
                        "While inserting object value tables: environment variable '{}' does not exist.".format(
                            name
                        )
                    )
                    continue
            ...
            self.session.add(vt)
            self.session.flush()
            ovt = Object_Valuetable(
                object_type=otype, object_rid=object_rid, valuetable_id=vt.rid
            )
    ...
    def exitEnvironmentVariables(self, ctx):
        ...
        for var in ctx.value:
            ...
            for node in accessNodes:
                ...
                if not nn:
                    self.logger.error(
                        "While inserting environment variables: node '{}' does not exist.".format(
                            name
                        )
                    )
                    continue
    ...
    def exitEnvironmentVariable(self, ctx):
        ...
        ctx.value = dict(
            ...
            varType=self.getValue(ctx.varType),
            minimum=self.getValue(ctx.minimum),
            maximum=self.getValue(ctx.maximum),
            unit=self.getValue(ctx.unit),
            initialValue=self.getValue(ctx.initialValue),
            envId=self.getValue(ctx.envId),
            accessType=accessType,
            accessNodes=self.getValue(ctx.accNodes),
        )
    ...
    def exitEnvironmentVariableData(self, ctx):
        ctx.value = dict(
            name=self.getValue(ctx.varname), value=self.getValue(ctx.value)
        )
    ...
    def exitSignalType(self, ctx):
        ctx.value = dict(
            name=self.getValue(ctx.signalTypeName),
            size=self.getValue(ctx.signalSize),
            byteOrder=self.getValue(ctx.byteOrder),
            valueType=self.getValue(ctx.valueType),
            factor=self.getValue(ctx.factor),
            offset=self.getValue(ctx.offset),
            minimum=self.getValue(ctx.minimum),
            maximum=self.getValue(ctx.maximum),
            unit=self.getValue(ctx.unit),
            defaultValue=self.getValue(ctx.defaultValue),
    ...
    def exitComments(self, ctx):
        ...
        for comment in ctx.value:
            ...
            if tp == "BU":
                ...
                if not obj:
                    self.logger.error(
                        "While inserting comments: node '{}' does not exist.".format(
                            key
                        )
                    )
                    continue
                ...
                obj = (
                    self.MESSAGE_BY_MESSAGE_ID(self.session)
                    .params(message_id=key)
                    .first()
                )
                if not obj:
                    self.logger.error(
                        "While inserting comments: message '{}' does not exist.".format(
                            key
                        )
                    )
                    continue
                ...
                if not rid:
                    self.logger.error(
                        "While inserting comments: message signal '{}' does not exist.".format(
                            key
                        )
                    )
                    continue
                ...
                if not obj:
                    self.logger.error(
                        "While inserting comments: environment variable '{}' does not exist.".format(
                            key
                        )
                    )
                    continue
                ...
                obj = (
                    self.NETWORK_BY_RID(self.session)
                    .params(rid=self.network_id)
                    .first()
                )
                if not obj:
                    self.logger.error(
                        "While inserting comments: network '{}' does not exist.".format(
                            self.network_id
                        )
                    )
                    continue
    ...
    def exitComment(self, ctx):
        ...
        if ctx.c0:
            ...
            tp = "SG"
            key = (
                self.getValue(ctx.i2),
                self.getValue(ctx.c2),
            )
        elif ctx.c3:
            tp = "EV"
    ...
    def exitAttributeValueType(self, ctx):
        if ctx.i00:
            tp = "INT"
            value = (
                self.getValue(ctx.i00),
                self.getValue(ctx.i01),
            )
        elif ctx.i10:
            tp = "HEX"
            value = (
                self.getValue(ctx.i10),
                self.getValue(ctx.i11),
            )
        elif ctx.f0:
            tp = "FLOAT"
            value = (
                float(self.getValue(ctx.f0)),
                float(self.getValue(ctx.f1)),
            )
        elif ctx.s0:
            tp = "STRING"
            value = None
    ...
    def exitAttributeValues(self, ctx):
        ...
        for attr in ctx.value:
            ...
            if not ad:
                self.logger.error(
                    "While inserting attribute values: attribute definition named '{}' does not exist.".format(
                        attr["name"]
                    )
                )
                continue
            ...
            if attrType == AttributeType.MESSAGE:
                ...
                if not msg:
                    self.logger.error(
                        "While inserting attribute values: message '{}' does not exist.".format(
                            key
                        )
                    )
                    continue
                ...
                if not node:
                    self.logger.error(
                        "While inserting attribute values: node '{}' does not exist.".format(
                            key
                        )
                    )
                    continue
                ...
                if not envvar:
                    self.logger.error(
                        "While inserting attribute values: environment variable '{}' does not exist.".format(
                            key
                        )
                    )
                    continue
                ...
                key = ""
            exists = (
                self.EXISTS_ATTRIBUTE_VALUE(self.session)
                .params(rid=rid, ad=ad.rid)
                .first()
            )
            if exists:
                self.logger.error(
                    "While inserting attribute values: attribute value for {} {} {} already exists.".format(
                        AttributeType.SIGNAL.name, key, ad.name
                    )
                )
            else:
                av = Attribute_Value(
    ...
    def exitRelativeAttributeValues(self, ctx):
        ...
        for attr in items:
            ...
            ad = (
                self.ATTRIBUTE_DEFINITION_BY_NAME(self.session)
                .params(name=attr["attributeName"])
                .first()
            )
            if not ad:
                self.logger.error(
                    "While inserting relative attribute values: attribute definition '{}' does not exist.".format(
                        attr["attributeName"]
                    )
                )
                continue
            ...
            if not ad:
                self.logger.error(
                    "While inserting relative attribute values: node '{}' does not exist.".format(
                        attr["nodeName"]
                    )
                )
                continue
            ...
            if attrributeType == AttributeType.REL_SIGNAL:
                ...
                if not rid:
                    self.logger.error(
                        "While inserting relative attribute values: message signal '{}'::'{}' does not exist".format(
                            messageID, parent["signalName"]
                        )
                    )
                    continue
                ...
                if not env_var:
                    self.logger.error(
                        "While inserting relative attribute values: environment variable '{}' does not exist.".format(
                            evName
                        )
                    )
                    continue
                ...
                if not msg:
                    self.logger.error(
                        "While inserting relative attribute values: message '{}' does not exist.".format(
                            messageID
                        )
                    )
                    continue
                rid = msg.rid
            arv = AttributeRel_Value(
                object_id=rid,
                attribute_definition=ad,
                num_value=numValue,
                string_value=stringValue,
                opt_object_id_1=optOid1,
                opt_object_id_2=optOid2,
            )
    ...
    def exitSignalGroups(self, ctx):
        ...
        for group in ctx.value:
            ...
            if not msg:
                self.logger.error(
                    "While inserting signal groups: message '{}' does not exist.".format(
                        messageID
                    )
                )
                continue
            ...
            for signalName in signalNames:
                signal = (
                    ...
                    .params(messageID=messageID, signalName=signalName)
                    .first()
                )
                if not signal:
                    self.logger.error(
                        "While inserting signal groups: message signal '{}'::'{}' does not exist.".format(
    ...
    def exitSignalGroup(self, ctx):
        ...
        ctx.value = dict(
            messageID=messageID, groupName=groupName, gvalue=gvalue, signals=signals
        )
    ...
    def exitCategoryDefinitions(self, ctx):
        ...
        for category in ctx.value:
            ...
            cd = Category_Definition(
                name=category["name"], key=category["category"], level=category["value"]
            )
            self.session.add(cd)
        self.session.flush()
    ...
    def exitCategories(self, ctx):
        ...
        for category in ctx.value:
            ...
            if attrType == "BU":
                ...
                if not node:
                    self.logger.error(
                        "While inserting categories: node '{}' does not exist.".format(
                            nodeName
                        )
                    )
                    continue
                ...
                if not msg:
                    self.logger.error(
                        "While inserting categories: message '{}' does not exist.".format(
                            messageID
                        )
                    )
                    continue
                ...
                if not env_var:
                    self.logger.error(
                        "While inserting categories: environment variable '{}' does not exist.".format(
                            envVarname
                        )
                    )
                    continue
                rid = env_var.rid
            cv = Category_Value(
                object_id=rid, category_definition_id=catId, objecttype=objType
            )
            self.session.add(cv)
            print(cv)
        self.session.flush()
